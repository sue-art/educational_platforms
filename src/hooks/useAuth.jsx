...
export const AuthProvider = ({ children }) => {
  ...
  const register = async (userData) => {
    ...
    try {
      ...
      // Validate username (mock validation)
      if (userData.username === "test") {
        throw new Error("Username already exists");
      }

      // If successful, you might want to:
      // 1. Either automatically log the user in
      // 2. Or return success and redirect to login page

      // For this example, we'll just return success
      return { success: true };
    } catch (e) {
      setError(e.message || "Registration failed");
      throw e; // Re-throw to allow component to handle registration error
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const value = {
    ...
    login,
    logout,
    register, // Added the register function to the context value
  };
...
// Export the context itself if needed for direct consumption or testiAuthContextng
export { AuthContext };
